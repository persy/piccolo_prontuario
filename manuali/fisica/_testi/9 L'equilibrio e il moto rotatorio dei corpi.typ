...
In generale, affinché un corpo non ruoti, è necessario che i momenti delle forze che tendono a farlo ruotare in un senso siano bilanciati dai momenti delle forze che tendono a farlo ruotare nel senso opposto. In questo modo, il momento totale risulta nullo. Per soddisfare questa condizione, devono essere rispettate le seguenti due regole affinché un corpo rigido sia in equilibrio: la somma delle forze applicate deve essere uguale a zero e la somma dei momenti delle coppie applicate deve anch'essa risultare pari a zero.

La prima condizione impedisce al corpo di traslare, mentre la seconda impedisce che esso ruoti.

Il momento M di una forza F, applicata in un punto P rispetto a un punto generico O, si calcola come il prodotto dell'intensità della forza F e della distanza tra il punto di applicazione della forza e il punto O, attraverso cui passa l'asse di rotazione. Tale distanza è rappresentata dal segmento OP, chiamato anche braccio della forza.

La direzione del vettore M è perpendicolare alla forza e al segmento OP; il suo verso, invece, si determina mediante la regola della mano destra. Il momento di una forza quantifica la capacità di quella forza di determinare la rotazione di un oggetto rispetto al punto O. Per aumentare il momento di una forza, è sufficiente incrementare il suo braccio.
...
Un'altra tipologia di equilibrio riguarda i corpi appoggiati. Un corpo pesante appoggiato è vincolato a una superficie e soggetto alla forza di gravità e alla forza vincolare, che agisce verso l'alto grazie alla base d'appoggio.
...
Un corpo pesante appoggiato è vincolato a una superficie e soggetto alla forza di gravità, controbilanciata dalla forza vincolare diretta verso l'alto, fornita dalla base di appoggio.

Nel caso di appoggio su un singolo punto, l'equilibrio si verifica quando il baricentro del corpo si trova sulla verticale del punto di appoggio. Anche in questa configurazione possono manifestarsi tre tipi di equilibrio: stabile, instabile e indifferente.

- Equilibrio stabile: si ha quando, spostando l'oggetto dalla sua posizione di equilibrio, il baricentro tende a salire. In questa condizione, il corpo ritorna spontaneamente alla posizione iniziale.  
- Equilibrio instabile: si verifica quando lo spostamento dell'oggetto porta il baricentro a scendere. Il corpo si allontana dalla posizione originale senza tornare indietro.  
- Equilibrio indifferente: si osserva quando il baricentro resta alla medesima quota dopo lo spostamento. Il corpo mantiene la nuova posizione raggiunta.
...
Le condizioni di equilibrio stabile, instabile e indifferente possono essere analizzate anche dal punto di vista dell'energia potenziale.

- Un corpo è in equilibrio stabile quando, se leggermente spostato dalla sua posizione iniziale, tende naturalmente a ritornarvi.  
- È in equilibrio instabile se, dopo un piccolo spostamento dalla sua posizione di equilibrio, si allontana ulteriormente.  
- Infine, è in equilibrio indifferente quando un lieve spostamento lo porta semplicemente a stabilizzarsi nella nuova posizione senza variazioni sostanziali.  
...
dove \(M\) è il momento delle forze esterne che agiscono sul corpo e \(\alpha\) rappresenta l'accelerazione angolare, ossia la variazione della velocità angolare nel tempo.

Analogamente alla quantità di moto per il moto traslatorio, nel moto rotatorio si definisce il momento angolare \(P\) (o momento della quantità di moto), una grandezza vettoriale che descrive il moto di rotazione del corpo.

Per un corpo rigido, il momento angolare risulta orientato lungo l'asse di rotazione e la sua intensità è determinata dal prodotto tra la velocità angolare del corpo e il momento di inerzia. Analogamente alla quantità di moto (il cui modulo è dato dalla relazione p = mv), è valida una legge di conservazione del momento angolare. Essa afferma che, in assenza di un momento risultante derivante da forze esterne su un corpo o su un sistema isolato di corpi, il momento angolare del corpo o del sistema rimane costante.
...
Dove \( I \) rappresenta il momento di inerzia del corpo e \( \omega \) la sua velocità angolare.
...
Il principio fondamentale delle leve si basa sull'equilibrio rotatorio. Questo equilibrio si raggiunge eguagliando i momenti delle forze applicate; pertanto, è sufficiente aumentare il braccio d'azione di una forza meno intensa (modificando la distanza tra le forze agenti e il fulcro) per aumentare il suo momento e contrastare una forza più intensa.

Le leve possono essere classificate in tre categorie principali: leve di primo, secondo e terzo genere. Tale classificazione dipende dalla posizione reciproca del fulcro, della potenza e della resistenza.
