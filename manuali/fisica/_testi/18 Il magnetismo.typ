...
Il grado di magnetizzazione di una sostanza viene espresso dalla permeabilità magnetica relativa (μr).
...
La calamita, termine con il quale si indicano in linguaggio comune i magneti permanenti, è una varietà della magnetite.

Le proprietà attrattive della calamita sono concentrate alle sue estremità, mentre nella zona centrale una calamita è neutra.

In genere una calamita è sagomata a ferro di cavallo o, più comunemente, a barretta: le sue estremità vengono dette poli magnetici.

I poli magnetici di una calamita attirano la limatura di ferro o piccoli pezzetti di ferro posti nelle sue vicinanze.
...
Se avviciniamo tra loro i poli nord (o sud) di due calamite, questi tenderanno a respingersi.

Allo stesso modo in cui cariche elettriche dello stesso segno si respingono e cariche elettriche di segno opposto si attraggono.

Ma l'analogia con le cariche elettriche si ferma qui, poiché nel caso dei magneti, a differenza di quanto accade per le cariche elettriche, non esistono poli isolati.

Questo significa che un magnete è sempre composto di un polo nord e di un polo sud.

Se proviamo a tagliare un magnete al centro, sperando di dividere tra loro i due poli, otterremo soltanto un magnete più piccolo, ma alle due estremità vi saranno sempre un polo nord e un polo sud.

Anche riducendo il magnete a dimensioni microscopiche si ottengono sempre due poli, o dipolo: è impossibile ottenere un polo magnetico isolato, o un monopolo.

== Il campo magnetico

Se, avvicinando due magneti, questi si attraggono o si respingono a seconda dell'orientamento dei rispettivi poli, significa che esercitano l'uno sull'altro una forza a distanza.

Come per i fenomeni elettrici e per le forze gravitazionali, risulta utile descrivere le interazioni magnetiche utilizzando il concetto di campo.
...
Come per gli altri campi vettoriali, le linee di forza del campo magnetico sono tangenti alla direzione del campo in ogni punto e sono tanto più fitte quanto più elevata è l'intensità del campo.

Le linee di forza del campo magnetico prodotto da un magnete sono visualizzabili con un'esperienza molto semplice.
...
Un sistema di questo tipo viene chiamato elettromagnete.

Gli elettromagneti sono molto usati nelle applicazioni tecnologiche: la forza di attrazione che possono esercitare dipende dal numero di avvolgimenti e dall'intensità della corrente che circola nelle spire e può raggiungere valori estremamente elevati.
...
A questo scopo si considera un tratto di filo di lunghezza l, percorso da una corrente I, posto tra i poli di una calamita, ovvero immerso in un campo magnetico.
...
Se si misura la forza F con un dinamometro, questa risulta proporzionale alla lunghezza del filo l e all'intensità della corrente I:dove la costante di proporzionalità B rappresenta l'intensità del vettore campo magnetico.
...
L'unità di misura dell'intensità del campo magnetico nel Sistema Internazionale è il tesla (simbolo T).

Si dice che un campo magnetico ha intensità di 1 tesla quando esercita una forza di 1 newton su un filo conduttore della lunghezza di 1 m percorso da una corrente di 1 ampere:La forza esercitata da un campo magnetico su un filo percorso da corrente è sfruttata nei motori elettrici per trasformare energia elettrica in energia meccanica.

=== Intensità del campo magnetico generato da un filo percorso da corrente

L'intensità del campo magnetico generato da un filo rettilineo percorso da una corrente di intensità I è inversamente proporzionale alla distanza d dal conduttore e direttamente proporzionale alla corrente:dove la costante di proporzionalità k si scrive per convenzione k =0/2, con0 che rappresenta la permeabilità magnetica nel vuoto e vale0 = 12,56·10-7Tm/A.

=== Intensità del campo magnetico generato da una spira percorsa da corrente

Nel caso di una spira di raggio r percorsa da una corrente I, il campo magnetico ha intensità direttamente proporzionale alla corrente e inversamente proporzionale al raggio della spira r:e la costante di proporzionalità k è ancora quella del caso precedente.
...
In questo caso la costante di proporzionalità dipende dal mezzo posto all'interno del solenoide; se al suo interno c'è aria (che per comodità supporremo avere una permeabilità magnetica paragonabile a quella del vuoto), la costante di proporzionalità sarà semplicemente=0.

Se all'interno del solenoide viene posto un mezzo materiale con permeabilità magnetica relativa pari ar, la costante diventa=0?r e il campo diventa tanto più intenso quanto più alta è la permeabilità magneticar del mezzo.
...
Questa forza è la forza di Lorentz, così detta dal nome del fisico olandese Hendrik Antoon Lorentz (1853-1928), premio Nobel per la fisica nel 1902.

Per calcolare l'intensità della forza di Lorentz partiamo dalla forza esercitata su un tratto di filo di lunghezza l percorso da una corrente I:Una particella con carica q che si muove in quel tratto di filo produce una corrente data dalla sua intensità di carica nell'unità di tempo, quindi:

Se la particella si muove con velocità uniforme v, percorre un tratto l in un tempo t e la corrente si può scrivere:così l'intensità della forza di Lorentz diventa:
...
Essendo sempre perpendicolare alla direzione della particella carica, la forza di Lorentz funge da forza centripeta per una particella di velocità perpendicolare alla direzione del campo magnetico e costringe la particella a curvare la sua traiettoria lungo un percorso circolare, il cui raggio r si può ricavare uguagliando la forza di Lorentz alla forza centripeta:Questo effetto è sfruttato negli acceleratori di particelle ad anello, che, attraverso l'uso di campi magnetici, costringono le particelle cariche a rimanere confinate su traiettorie circolari.
...
In genere le bussole sono dotate di un quadrante suddiviso in 360 gradi, sul quale vengono segnati i punti cardinali, così da permettere l'orientamento.

Sembra che la bussola fosse già nota in Cina (VIII sec.) e in Arabia (XI sec.) prima di divenire di uso comune in Occidente, ma non ci sono fonti sicure a questo riguardo.

La prima descrizione dettagliata della bussola e del suo funzionamento risale al 1269 ed è dovuta a uno studioso francese, noto come Peter Peregrinus, il quale dette anche il nome di polo sud e di polo nord alle due estremità dell'ago magnetico.

La bussola fu perfezionata e diffusa soprattutto dagli amalfitani (Flavio Gioia) nel XIV sec. == Appr. Il motore elettrico

In un motore elettrico l'energia elettrica fornita a una spira percorsa da corrente viene trasformata in energia meccanica.

L'energia meccanica così prodotta può poi venire utilizzata per muovere per esempio le pale di un ventilatore o il cestello di una lavatrice.Un motore elettrico è costituito generalmente da una parte fissa (statore) e da una mobile (rotore).

Una spira rettangolare di filo conduttore rigido, montata su un asse sul quale è libera di ruotare, è immersa in un campo magnetico uniforme perpendicolare all'asse di rotazione, generato per esempio dai poli di un magnete.

Quando la spira è percorsa da corrente elettrica l'azione del magnete produce una coppia di forze che la costringono a ruotare.
...
Se a questo punto, mediante uno speciale dispositivo detto collettore, si inverte il senso della corrente nella spira, viene invertita anche l'azione delle forze e la spira continua la rotazione nello stesso senso.

La rotazione della spira viene trasmessa a un albero girevole e può far funzionare un apparecchio elettrico.

Un motore elettrico reale in genere è un po' più complesso di quello descritto (può essere costituito per esempio da bobine, anziché da una singola spira nella parte mobile), ma il principio di funzionamento rimane invariato.
